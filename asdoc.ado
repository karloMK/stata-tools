...
*!-------------------------------------------------------------------------------
*! Karlo custom patch 2026-09-21: pwcorr star(all) uses *** p<.001, ** p<.01, * p<.05
...
prog asdoc, byable(onecall) 
	...
	else global setstars "***@.001, **@.01, *@.05"
	if "`dec'" 		== "" loc dec 3
	loc opsys `c(os)'
...
program define asdocor, byable(recall)
		...
		while `i'<=`nvar' {
			...
			while (`j'<=min(`j1',`i')) {
				if "`star'" == "all" {
					...
					* Custom publication-style significance stars for pwcorr:
					* *** p < .001, ** p < .01, * p < .05
					if `i' != `j' {
						if `p`j'' < .001 local ast "***"
						else if `p`j'' < .01 local ast "**"
						else if `p`j'' < .05 local ast "*"
						else local ast " "
					}
					else local ast " "
	...
	if "`star'" != "-1" {
		if "`star'" == "all" {
			loc siginfo "\i *** p<.001, ** p<.01, * p<.05"
		}
		else {
			loc siginfo "\i * p<`star'"
		}
